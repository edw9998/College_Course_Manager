...
def view_course_history_page():
    ...
    myCursor.execute("SELECT * FROM Course_History")
    ...
    # Define the columns.
    tree["columns"] = ("participant_id", "course_id")

    # Assign column width, min-width attributes & align column names to center.
    all_widths = 333
    tree.column("participant_id", width = all_widths, minwidth = all_widths, anchor = CENTER)
    tree.column("course_id", width = all_widths, minwidth = all_widths, anchor = CENTER)
    ...
    start = 0
    for row in myCursor:
        tree.insert("", start, text = "", values = (row[0], row[1]))
        start = start + 1
